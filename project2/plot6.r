...
ABYT <- aggregate(Emissions ~ year + fips, subsetNEI, sum)
ABYT$fips[ABYT$fips=="24510"] <- "Baltimore, MD"
ABYT$fips[ABYT$fips=="06037"] <- "Los Angeles, CA"

png("plot6.png", width=1040, height=480)
g <- ggplot(ABYT, aes(factor(year), Emissions))
g <- g + facet_grid(. ~ fips)
